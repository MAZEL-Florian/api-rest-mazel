...
async function updateOrCreatePlate() {
  if (route.params.plate_id) {
    ...
    const restaurant = await ApiService.getLoggedUser();
    plate.value.restaurant_id = restaurant.restaurant;
...
<template>
  <div>
    ...
    <div class="form">
      ...
      <input v-model="plate.urlPhoto" type="text" placeholder="Photo Url" />
      <input v-model="plate.prix" type="text" placeholder="Prix" />
